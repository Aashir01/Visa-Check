...
The grading rolls up into an operator dashboard: a per-ground scoreboard sorted
worst-catch-rate-first, which is the maintenance queue for the rule packs. That
is the moat made operational — not "we have good rules", but a standing measure
of exactly where they are weakest, refreshed by every refusal that comes in.
